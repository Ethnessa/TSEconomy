...
namespace Examples
{
    ...
    {
        // we are going to find the TShock user 'johndoe' and take 50 'dollars' away from him
        public void Example()
        {
            // fetch user john doe
            ...
            bool success = bankAccount.TryRemoveBalance(50.0);

            if (success)
            {
                TShock.Log.ConsoleInfo("Success!");
            }
            else
            {
                TShock.Log.ConsoleError("That transaction could not be completed, the player did not have enough money!");
            }
